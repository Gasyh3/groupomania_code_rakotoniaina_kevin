        ...
        bcrypt
          ...
          .then((comparedPassword) => {
            ...
            const token = jwt.sign(
              { userId: result[0].id, privilege: result[0].privileges },
              "token",
              { expiresIn: "72h" }
            );
              ...
              User.modifyUser(userModifications, (err, data) => {
                if (err)
                  res.status(500).send({
                    message:
                      err.message ||
                      "Some error occurred while modifying the password.",
                  });
              });
              res
                .status(200)
...
exports.modifyUser = (req, res) => {
  ...
  function checkAndSendUserModifications(reqBody, userModifications) {
    for (value in reqBody) {
      //check and add only valid values
          ...
          if (req.file !== undefined) {
            ...
            sql.query(
              ...
              (err, resp) => {
                ...
                fs.unlink(`images/${resp[0].avatarURL}`, () =>
                  console.log("previous user avatar has been delete")
                );
              }
            );
          }
