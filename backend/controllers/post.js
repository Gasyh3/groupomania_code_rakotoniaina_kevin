...
exports.getAllPosts = (req, res) => {
  ...
  sql.query(
    "SELECT p.title, p.text, p.mediaUrl, p.published_at, users.avatarURL, users.username, p.id AS postID FROM posts p JOIN users ON p.userID = users.id",
    (err, resp) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
...
exports.modifyPost = (req, res) => {
  sql.query(`SELECT * FROM posts WHERE id = ?`, req.body.id, (err, resp) => {
    ...
    if (req.userID !== resp[0].userID) {
      ...
      if (req.file !== undefined) {
        fs.unlink(`images/${resp[0].mediaUrl}`, () =>
          console.log("previous image has been delete")
        );
...
exports.deletePost = (req, res) => {
  ...
  sql.query(`SELECT * FROM posts WHERE id = ?`, req.body.id, (err, resp) => {
    ...
    if (req.userPrivilege == 1 || req.userId == resp[0].userID) {
      fs.unlink(`images/${resp[0].mediaUrl}`, () =>
        console.log("post image has been delete")
      );
